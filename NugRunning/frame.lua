...
function TimerBar.SetColor(self,r,g,b)
	...
	-- self.bar.bg:SetVertexColor(r*.5, g*.5, b*.5)	-- Xyz					
end
...
function TimerBar:UpdateFrameBorder(borderType)
    ...
    if borderType == "2PX" then
		...
		-- backdrop:SetTexture("Interface\\BUTTONS\\WHITE8X8") -- Xyz
        -- backdrop:SetVertexColor(0,0,0, 0.5) -- Xyz
...
NugRunning.ConstructTimerBar = function(width, height)
    ...
    local texture = getStatusbar()
	-- bar:SetStatusBarTexture(texture) -- Xyz
    -- bar:GetStatusBarTexture():SetDrawLayer("ARTWORK") -- Xyz
    bar:SetHeight(height)
    bar:SetWidth(width - height - 1)
    bar:SetPoint("TOPRIGHT",f,"TOPRIGHT",0,0)

	-- bar.bg = bar:CreateTexture(nil, "BORDER") -- Xyz
									
	-- bar.bg:SetAllPoints(bar) -- Xyz
    -- bar.bg:SetTexture(texture) -- Xyz

    local spark = bar:CreateTexture(nil, "ARTWORK", nil, 4)
    spark:SetBlendMode("ADD")
	-- spark:SetTexture([[Interface\AddOns\NugRunning\spark.tga]]) -- Xyz
    ...
    f.timeText:SetPoint("LEFT", f.bar, "LEFT",3,0)
    ...
    f.spellText:SetAlpha(nameAlpha or 1)
    f.spellText:SetPoint("RIGHT", f.bar, "RIGHT",0,0)

    local overlay = f.bar:CreateTexture(nil, "ARTWORK", nil, 3)
	-- overlay:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background") -- Xyz
	...
	-- arrow:SetTexture("Interface\\BUTTONS\\WHITE8X8") -- Xyz
	...
	-- sbg:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background") -- Xyz
